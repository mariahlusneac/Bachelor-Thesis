...
path_portraits = ''
# path_archives = path_portraits + 'dataset/dataset_arrays_archives_to_send/'
...
for sc in skip_conn:
  for upscale_method in deconv_upscale_method:
    autoenc_models.append(autoencoder(input_shape, sc, conv_batchnorm, 
                                      ...
                                      model_loss))
# autoenc_models contains 4 models
...
class CelebASequenceT(Sequence):
  ...
  def __getitem__(self, idx):
    ...
    if self.batch_index % self.nr_batches_per_zip == 0:
      self.x_archive = np.load(path_archives + 'x_{}.npz'.format(int(archive_idx+1)))
      self.x_archive = self.x_archive['x_train']
      self.y_archive = np.load(path_archives + 'y_{}.npz'.format(int(archive_idx+1)))
      self.y_archive = self.y_archive['y_train']
      # preprocessing
      self.x_archive = np.asarray(self.x_archive, dtype=np.float64)
      self.x_archive = self.x_archive / 255.0
      self.y_archive = np.asarray(self.y_archive, dtype=np.float64)
      self.y_archive = self.y_archive / 255.0
...
def train_autoenc(train_number, batch_size, nr_archives_train, len_dataset, model_index, path_archives):
  ...
  x_val = x_val['x_val']/255.0
  y_val = np.load(path_archives + 'y_val_all.npz')
  y_val = y_val['y_val']/255.0
...
def discriminator_basic(image_shape_bw, image_shape_rgb, loss_weights):
  ...
  model.compile(loss='binary_crossentropy', optimizer=Adam(lr=0.0001, beta_1=0.5), loss_weights=loss_weights)
  return model

image_shape_bw = (216, 176, 1)
image_shape_rgb = (216, 176, 3)
loss_weights = [0.5]
...
def train_gan(train_number, model_index_gan, path_dataset, len_dataset, nr_archives, nr_epochs, batch_size, dim1_patch, dim2_patch):
  ...
  for i in tqdm(range(nr_iterations)):
    ...
    x_real = np.load(path_dataset + 'x_{}.npz'.format(idx))['x_train'][idx_start:idx_stop]/255.0
    y_real = np.load(path_dataset + 'y_{}.npz'.format(idx))['y_train'][idx_start:idx_stop]/255.0
    ...
    # train discr with bw images
    loss1_discr = discr.train_on_batch([x_real, y_real], y_real_patch)
    # train discr with rgb generated images
...
# path_archives = path_portraits + 'dataset/dataset_arrays_archives_to_send/'
path_portraits = ''
path_archives = path_portraits + 'archives/'
len_dataset = 32768
nr_archives = 32
nr_epochs = 100
batch_size = 32
