...
def train_autoenc(train_number, batch_size, nr_archives_train, len_dataset, model_index, path_archives):
  ...
  x_val = np.load(path_archives + 'x_val_all_small.npz')
  x_val = x_val['x_val']/255
  y_val = np.load(path_archives + 'y_val_all_small.npz')
...
def encode_layers_gen(prev_layer, nr_filters, kernel_size, stride, batchnorm):
...
def decode_layers_gen(prev_layer, nr_filters, kernel_size, stride, dropout, skip_conn, upsample_method, *to_concat_with):
...
def generator(image_shape, skip_conn, deconv_upsample_method):
    ...
    e1 = encode_layers_gen(in_image, 64, kernel_size=(3,3), stride=(2,2), batchnorm=False)
    e2 = encode_layers_gen(e1, 128, kernel_size=(3,3), stride=(1,1), batchnorm=False)
    e3 = encode_layers_gen(e2, 128, kernel_size=(1,1), stride=(2,2), batchnorm=True)
    e4 = encode_layers_gen(e3, 256, kernel_size=(3,3), stride=(1,1), batchnorm=True)
    e5 = encode_layers_gen(e4, 256, kernel_size=(1,1), stride=(2,2), batchnorm=True)
    e6 = encode_layers_gen(e5, 512, kernel_size=(3,3), stride=(1,1), batchnorm=True)

    b = Conv2D(512, (4, 4), strides=(1, 1), padding='same')(e6)
    b = Activation('relu')(b)
                           
    d6 = decode_layers_gen(b, 512, (1,1), (1,1), False, skip_conn, deconv_upsample_method, e6)
    d5 = decode_layers_gen(d6, 256, (3,3), (1,1), False, skip_conn, deconv_upsample_method, e5)
    d4 = decode_layers_gen(d5, 256, (1,1), (2,2), True, skip_conn, deconv_upsample_method, e4)
    d3 = decode_layers_gen(d4, 128, (3,3), (1,1), True, skip_conn, deconv_upsample_method, e3)
    d2 = decode_layers_gen(d3, 128, (1,1), (2,2), True, skip_conn, deconv_upsample_method, e2)
    d1 = decode_layers_gen(d2, 64, (3,3), (1,1), True, skip_conn, deconv_upsample_method, e1)
